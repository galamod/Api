                    ...
                    contentTypeHeader.Contains("application/manifest+json"))) // Добавляем manifest
                {
                    ...
                    else if (contentTypeHeader.Contains("javascript"))
                    {
                        ...
                        if (path.StartsWith("web/app.", StringComparison.OrdinalIgnoreCase) && path.EndsWith(".js"))
                        {
                            ...
                            // Находим все "безопасные" точки для внедрения (после точки с запятой)
                            var injectionPoints = new List<int>();
                            for (int i = 0; i < text.Length; i++)
                            {
                                if (text[i] == ';')
                                {
                                    injectionPoints.Add(i + 1);
                                }
                            }

                            if (injectionPoints.Any())
                            {
                                // Выбираем случайную точку для внедрения
                                var random = new Random();
                                var index = random.Next(injectionPoints.Count);
                                var injectionPoint = injectionPoints[index];

                                text = text.Insert(injectionPoint, automationScript);
                                _logger.LogInformation("✅ Automation script injected into {Path} at position {Position}", path, injectionPoint);
                            }
                            else
                            {
                                // Если безопасных точек не найдено, добавляем в конец (запасной вариант)
                                text += automationScript;
                                _logger.LogInformation("✅ Automation script appended to {Path}", path);
                            }
                        }
                    }
