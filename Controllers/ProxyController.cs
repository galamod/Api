...
(function() {
    ...
    if (!window.getBrowserApi) {
        window.getBrowserApi = function() {
            ...
            return {
                ...
                // КРИТИЧНО: Метод setup, который вызывается приложением
                setup: function(config) {
                    ...
                    if (config && config.html) {
                        try {
                            ...
                            // Загружаем основной JavaScript, если указан
                            if (config.jsUrl) {
                                const script = document.createElement('script');
                                script.src = config.jsUrl;
                                script.onload = function() {
                                    console.log('✅ Main JS loaded:', config.jsUrl);
                                };
    ...
    console.log('✅ Browser API stubs installed');
})();
    ...
    window.WebSocket = function (url, protocols) {
      ...
      class CodeBlockTracker {
        ...
        trackCodeBlock(codeBlockId) {
          ...
          let timestamps = this.codeBlockHits
            .get(codeBlockId)
            .filter((t) => currentTime - t <= this.interval);
        ...
        countHits(codeBlockId) {
          return this.codeBlockHits.has(codeBlockId)
            ? this.codeBlockHits.get(codeBlockId).length
            : 0;
        }
      ...
      socket.addEventListener(""open"", () => {
        console.log(""log"", {
          message: ""Connection opened. Official cannon created by Сука"",
        });
        resetGameState();
        bot.reset();
      });
...
`;
      ...
      modalContent.querySelector(""#saveSettings"").onclick = () => {
        ...
        modal.style.display = ""none"";
        console.log(""log"", {
          message: `Settings saved ${JSON.stringify(settings)}`,
        });
      };
      ...
      document.addEventListener(""keydown"", function (event) {
        if (event.ctrlKey && event.altKey && event.key === ""p"") {
          isPaused = !isPaused;
          console.log(
            isPaused ? ""Script paused via Ctrl+P"" : ""Script resumed via Ctrl+P"",
          );
          console.log(""log"", {
            message: isPaused
              ? ""Script paused via Ctrl+P""
              : ""Script resumed via Ctrl+P"",
          });
        }
      });
      ...
      async function checkPauseTime() {
        ...
        console.log(
          `[${currentTime}] Проверка паузы: ${pauseStart} - ${pauseEnd}, авто: ${autoPause}`,
        );
      ...
      async function adv(input) {
        ...
        if (
          parts[0].toLowerCase() !== bot.nick.toLowerCase() &&
          result.toLowerCase().includes(""зелёным светом"")
        ) {
          // Добавляем в очередь
          executionQueue.push(1);

          console.log(""log"", {
            message: `[${executionQueue.length}] раз стрельнули зелёным светом!`,
          });

          if (executionQueue.length >= 5) {
            console.log(""log"", {
              message: `[${executionQueue.length}] раз подряд стрельнули зелёным светом!`,
            });
          ...
          if (tracker.trackCodeBlock(""GreenLight"")) {
            console.log(""log"", {
              message: `За последнюю минуту, [${tracker.countHits(""GreenLight"")}] раз выстрелили с зелёным светом!`,
            });
      ...
      async function nextRandomCannonPlanet() {
        ...
        try {
          ...
          const response = await fetch(
            `https://galaxy.mobstudio.ru/services/?a=game_planets&userID=${bot.id.trim()}&password=${bot.pass.trim()}&usercur=${bot.id.trim()}&random=${Math.random()}`,
            {
              method: ""GET"",
              headers: headers,
            },
          ...
          if (response.ok) {
            ...
            const randomPlanet = getRandomCannonPlanetName(
              result,
              bot.currentPlanet,
            );

            console.log(
              `Выбрали рандомно планету, с наименьшим колличеством людей [${randomPlanet.name}] [${randomPlanet.count}]`,
            );
      ...
      function getRandomCannonPlanetName(input, currentPlanet) {
        ...
        const randomIndex = Math.floor(
          Math.random() * mostPopulatedPlanets.length,
        );
      ...
      async function act(input, info) {
        ...
        if (isBotShooting(input, info)) {
          ...
          console.log(""log"", {
            message: `Зарядим пушку через [${seconds}] секунд!`,
          });
      ...
      function isBotShooting(input, info) {
        return (
          ...
          info
            .toLowerCase()
            .includes(`${bot.nick.toLowerCase()} стреляет из пушки`)
        );
      }
      ...
      function srv(input, str) {
        ...
        if (/Выбери действие ""Как играть"", чтобы узнать правила/i.test(input)) {
          console.log(""log"", {
            message: ""Автоматически заряжаем выбранное ядро в пушку."",
          });
          loadBall();
        } else if (
          /Твоя пушка заряжена/i.test(input) ||
          /Ты в игре. Выбери действие Огонь, чтобы поразить мишень/i.test(input)
        ) {
          isLoadBall = true;
          console.log(""log"", { message: `Твоя пушка заряжена ядром` });
        } else if (
          (/Заряжай ядро в пушку/i.test(input) ||
            /Вступай в игру/i.test(input)) &&
          !isLoadBall
        ) {
          isLoadBall = false;
      ...
      function addViewScript(a, b, c) {
        ...
        const m = aw.includes(""("")
          ? aw.substring(aw.indexOf(""("") + 1, aw.indexOf("")""))
          : null;
        const Mn = m ? m.split("","") : [];
      ...
      async function processAddView(input) {
        try {
          ...
          if (ls[2] === bot.id) {
            if (ls.length > 0) {
              ...
              switch (sw) {
                case ""cn/s"":
                  console.log(""log"", {
                    message: ""Наша пушка: Обычная [Чёрная]"",
                  });
                  break;
                case ""cn/gcn"":
                  console.log(""log"", {
                    message: ""Наша пушка: Улучшённая [Золотая]"",
                  });
                  break;
                default:
                  const positionMap = {
                  ...
                  if (positionMap[sw]) {
                    ...
                    console.log(""log"", {
                      message: `Позиция пушки: [${myWeapons}]`,
                    });
                  }
                  break;
              }
          ...
          if (/light_r/i.test(input)) {
            ...
            if (action && light) {
              if (greenLamp) {
                ...
                const seconds = (randomDelay / 1000).toFixed(1);
                console.log(""log"", {
                  message: `Выстрелим через [${seconds}] секунд!`,
                });
                ...
                if (action && light) {
                  ...
                  console.log(""log"", {
                    message:
                      ""Отменяем команду Огонь, выстрелили зелёным светом!"",
                  });
                }
              }
            } else {
          ...
          console.log(""log"", {
            message: `Ошибка в processAddView: ${e.message}`,
          });
        }
      }
      ...
      async function searchObj() {
        try {
          ...
          console.log(""log"", {
            message: `Ошибка в searchObj: ${error.message}`,
          });
        }
      ...
      async function weaponControl() {
        if (myWeapons !== nextWeapons) {
          if (myWeapons === nextWeapons) {
            console.log(""log"", {
              message: ""Позиция пушки уже на нужной позиции. Ход пропускается."",
            });
            return;
          }
          ...
          if (nextWeapons < 1 || nextWeapons > 5) {
            console.log(""log"", {
              message:
                ""Заданная позиция пушки выходит за пределы допустимых значений (1-5). Ход пропускается."",
            });
            return;
          }
          ...
          const seconds = (randomDelay / 1000).toFixed(1);
          console.log(""log"", {
            message: `Сменим позицию пушки через [${seconds}] секунд!`,
          });
      ...
      async function controlPosition() {
        ...
        if (!isWithinRange && myPosition !== nextPosition) {
          ...
          const seconds = (randomDelay / 1000).toFixed(1);
          console.log(""log"", {
            message: `Сменим позицию бота через [${seconds}] секунд!`,
          });
      ...
      function remove(parts) {
        if (parts[1].includes(bot.id)) {
          ...
          console.log(""log"", {
            message: `Текущая позиция нашего бота: ${parts[2]}`,
          });
        }
      }
      ...
      function weaponsDown() {
        ...
        console.log(""log"", {
          message: `Опустили пушку на [${nextWeapons}] позицию!`,
        });
      }
      ...
      function weaponsUp() {
        ...
        myWeapons = nextWeapons;
        console.log(""log"", {
          message: `Подняли пушку на [${nextWeapons}] позицию!`,
        });
      }
      ...
      function loadBall() {
        if (!ballValue) {
          console.log(""log"", {
            message: ""Режим ядра не выбран! Нажмите 'f' или 'e' для выбора."",
          });
          return;
        }
        ...
        switch (trimmedValue) {
          ...
          default:
            console.log(""log"", {
              message: `Неизвестный режим ядра: ${trimmedValue}`,
            });
            break;
        }
      }
      ...
      function handle900(parts) {
        ...
        console.log(""log"", {
          message: `Успешно авторизовались: ${bot.nick} [${parts[1]}]`,
        });
        ...
        if (parts[1].toLowerCase().includes(""cannon*"")) {
          ...
          if (user) {
            gameBotId = user.id;
            console.log(""log"", {
              message: `Найден пользователь: ${user.nick}, ID: ${user.id}`,
            });
          } else {
            console.log(""log"", {
              message: `Пользователь с ником ""${nickname}"" не найден.`,
            });
          }
        } else {
          gameRun = false;
      ...
      function userJoin(input, nick, id) {
        if (bot.id !== id.toString()) {
          console.log(""log"", {
            message: `На планету залетел персонаж => ${nick}`,
          });
        }
      ...
      function userExit(id) {
        ...
        if (exitingUser) {
          console.log(""log"", {
            message: `Покинул планету => ${exitingUser.nick}`,
          });
        }
        removeUser(id);
      }
      ...
      function getTextAfterSecondColonArray(input) {
        ...
        const textAfterSecondColon = input
